...
trnalldf = trnalldf[trnalldf.id_code != 'RPE-04_3_E04']

tstdf = tstdf[tstdf.id_code != 'HUVEC-18_3_D23']

tstdf = tstdf[tstdf.id_code != 'RPE-09_2_J16']
def keep_rows_with_img(df, dftype='train'):
    ...
    for t, row in df.iterrows():
        ...
        check = [os.path.exists(f'../input/recursion-cellular-image-classification/{dftype}/{row["experiment"]}/Plate{row["plate"]}/{row["well"]}_s1_w{channel+1}.png') for channel in range(6)]

        if sum(check)==6:

           pdls.append(pd.DataFrame(row).T)

        else:

            print(row)

    return pd.concat(pdls)
trnalldf.iloc[0]
# Remove the sirne sting form the file

trnalldf.sirna = trnalldf.sirna.apply(lambda x : int(x.split('_')[-1]))
...
class ImagesDS(D.Dataset):
    ...
    def __init__(self, df, img_dir, size = 256, mode='train', meandf = meanexpdf, stddf = stdexpdf, channels=[1,2,3,4,5,6]):
        ...
        self.site = 1 # random.randint(1,2) # load a random site from each well.
