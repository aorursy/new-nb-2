...
train = pd.read_csv('/kaggle/input/microsoft-malware-prediction/train.csv', dtype=dtypes, low_memory=True, nrows=100000)
train['MachineIdentifier'] = train.index.astype('uint32')
test  = pd.read_csv('/kaggle/input/microsoft-malware-prediction/test.csv',  dtype=dtypes, low_memory=True)
test['MachineIdentifier']  = test.index.astype('uint32')

gc.collect()
train[['Census_FirmwareManufacturerIdentifier', 'Census_FirmwareVersionIdentifier']].drop_duplicates()
train['Firmware_Cross'] = train[['Census_FirmwareManufacturerIdentifier', 'Census_FirmwareVersionIdentifier']].apply(lambda x: '.'.join([str(col) for col in x]), axis=1)
test['Firmware_Cross'] = test[['Census_FirmwareManufacturerIdentifier', 'Census_FirmwareVersionIdentifier']].apply(lambda x: '.'.join([str(col) for col in x]), axis=1)
useable_columns = list(filter(lambda x: x != 'HasDetections', train.columns.tolist()[1:]))
bad_columns = ['HasDetections', 'Platform', 'PuaMode']
useable_columns
print('Transform all features to category.\n')
for usecol in useable_columns:
...
submission = pd.read_csv('/kaggle/input/microsoft-malware-prediction/sample_submission.csv')
