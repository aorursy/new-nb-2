...
def get_model(cfg):
    ...
    outputs = []    
    for i in range(1,cfg['net_count']):
...
ds_train     = ds_train.map(lambda img, label: (img, tuple([label] * 6)))
...
for i in range(6):
    ...
    submission.to_csv(f'submission_model384B{i+1}.csv', index=False)
submission = pd.DataFrame(dict(
    image_name = image_names,
    target     = np.mean(probs[:,:,0], axis=0)))

submission = submission.sort_values('image_name') 
submission.to_csv('submission_model384AVERAGE.csv', index=False)
