...
#Не работает from sklearn.cross_validation import train_test_split
#Вроде подобрал работающий аналог
...
with zipfile.ZipFile("/kaggle/input/springleaf-marketing-response/test.csv.zip","r") as z:
    z.extractall(".")
with zipfile.ZipFile("/kaggle/input/springleaf-marketing-response/train.csv.zip","r") as z:
    z.extractall(".")
FEATURE_DTYPES = {
...
def print_memory_usage():
    print('consuming {:.2f}GB RAM'.format(
          ...
          flush=True)
#Возвращает dataframe из подсунутого csv файла, может возвращать определенные штуки
...
def get_train_X_df(train_csv_path='/kaggle/working/train.csv', n_rows_with_caption=None):
    return get_X_df(csv_file_path=train_csv_path, n_rows_with_header=n_rows_with_caption)


def get_train_y_values(train_csv_path='/kaggle/working/train.csv', n_rows_with_caption=None):
    return get_df(csv_path=train_csv_path, n_rows=n_rows_with_caption, column_names=['target']).values.ravel()


def get_test_X_df(test_csv_path='/kaggle/working/test.csv', n_rows_with_caption=None):
    return get_X_df(csv_file_path=test_csv_path, n_rows_with_header=n_rows_with_caption)


def get_test_id_df(test_csv_path='/kaggle/working/test.csv', n_rows_with_caption=None):
...
#Создается класс как объект, принимающий в себя значение key (типо x = DtypeSelector("Muda-Muda-Muda"))
#self = способ описания самого объекта (класса), типо ссылка на самого себя во время определения
class DTypeSelector(BaseEstimator, TransformerMixin):
    def __init__(self, key):
        self.key = key

    #Странно, нигде не применяется, по логике типо fit (если смотреть примеры в интернете по ML, где делают классы моделей с fit функцией) 
    #должен что то типо формировать линейную модель или что то в этом роде
...
print('{:.1f} minutes'.format((time.perf_counter() - perf_counter_start) / 60))
prediction_df.to_csv("xgboost_submission.csv", index=False)