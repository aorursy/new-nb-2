...
only_fatal=X_y_f.drop(['Iday1','Iday2','Iday3','Iday4','Iday5','Iday6','Iday7','target_infection','iratio','fratio'],axis=1)
y=only_fatal['target_fatal']
X=only_fatal.drop(['target_fatal'],axis=1)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33)
fatal=ElasticNet(random_state=42,l1_ratio=0.1,max_iter=2200)
params = [{'alpha': [10**-4,10**-3, 10**-2,10**-1, 10**0,10**1, 10**2,10**3,10**4]}]
...
print("RMSE :",mean_squared_error(y_test,pred_f)**0.5)
import math
Fatality=[]
countries=df.Country_Region.unique()
for country in countries:
...
test['Fatalities']=list(map(int,Fatality))
for country in ['US','Italy','Spain']:
