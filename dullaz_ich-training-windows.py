...
kfold = StratifiedKFold(n_splits=5, random_state=split_seed,shuffle=True).split(np.arange(pivot_df.shape[0]), pivot_df["any"].values)
...
dev_data = pivot_df.iloc[dev_idx]
...
TRAIN_STEPS = 100#(train_data.shape[0] // BATCH_SIZE)

VAL_STEPS = 100#dev_data.shape[0] // BATCH_SIZE
...
for PREP in ['WINDOW','BSB','SIGMOID_BSB','GRADIENT']:
