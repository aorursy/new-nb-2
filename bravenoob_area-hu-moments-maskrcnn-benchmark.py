...
area_train_df = pd.read_csv("../input/rcnn-all-train-images-results/result.csv")

area_test_df = pd.read_csv("../input/image-segmentation-test-data/test_result_df.csv")

hu_train_df = pd.read_csv("../input/hu-moments-maskrcnnbenchmark-sub/train_hu_moments_svd.csv")

hu_test_df = pd.read_csv("../input/hu-moments-maskrcnnbenchmark-sub/test_hu_moments_svd.csv")
area_train_df =  area_train_df.drop(['AdoptionSpeed'],axis=1)

hu_train_df = hu_train_df.drop(['AdoptionSpeed'], axis=1)
...
result_df = pd.merge(result_df, area_train_df, how='left', on='PetID')

result = pd.merge(result_df, hu_train_df, how='left', on='PetID')
result.shape
...
test_result_df = pd.merge(test_result_df, area_test_df, how='left', on='PetID')

test_result_df = pd.merge(test_result_df, hu_test_df, how='left', on='PetID')
submit=pd.DataFrame()
...
submit.to_csv('submission_ml.csv',index=False)
