...
collab_jac10 = collaborative_filter(s_train, s_input, sim_fn=jaccard_sim, j=54, k=5)

print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(collab_jac10), k=5)))

print("UHR = {0:.3f}".format(uhr_score(s_hidden, get_recs(collab_jac10), k=5)))
...
collab_cos10 = collaborative_filter(s_train, s_input, sim_fn=cosine_sim, j=60, k=5)

print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(collab_cos10), k=5)))

print("UHR = {0:.3f}".format(uhr_score(s_hidden, get_recs(collab_cos10), k=5)))
...
final_scores = collaborative_filter(s_train, s_all_input, sim_fn=cosine_sim, j=100)
