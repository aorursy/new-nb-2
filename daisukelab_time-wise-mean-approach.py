...
epochs = 50 # CHANGE TO 250, 50 is just for Kaggle kernel
...
for X in [b[0][:5]]:
    plot_dataset(X)
import keras
from keras.layers import Dense, Conv2D, MaxPooling2D, GlobalMaxPooling2D, AveragePooling2D, Activation, Dropout, BatchNormalization, Flatten
from keras.models import Sequential
from keras.preprocessing.image import ImageDataGenerator
from keras.callbacks import ModelCheckpoint, Callback, TensorBoard
...
model = model_cnn5(X_train.shape[1:])
...
callbacks = [
    ...
    CyclicLR(base_lr=0.00001, max_lr=0.0001, step_size=X_train.shape[0] // batch_size, mode='triangular'),
