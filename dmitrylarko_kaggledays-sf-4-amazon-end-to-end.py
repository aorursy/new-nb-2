...
def get_model(params, random_state = 42, ntrees = 250):
...
for s in seeds:
    ...
    skf = StratifiedKFold(n_splits=10, shuffle = True, random_state = s+1)
    ...
    for train_index, test_index in skf.split(data_train, y):
        ...
        model = get_model(params, random_state = s, ntrees = 2000)

        model.fit(

            data_train[train_index], y[train_index],

            eval_set = (data_train[test_index], y[test_index]), 

            early_stopping_rounds = 25, eval_metric = 'auc', verbose = False

        )

        tst_preds.append(model.predict_proba(data_test)[:,1].ravel())

    tst_preds = np.stack(tst_preds, -1).mean(-1)    

    

    predictions_1.append(tst_preds.ravel())
...
for s in seeds:
    ...
    for train_index, test_index in skf.split(data_train, y):
        ...
        tr_= train.iloc[train_index,:].reset_index(drop = True)

        val_ = train.iloc[test_index,:].reset_index(drop = True)

        

        te = TargetEncoding(columns_names = cols_te)

        data_te_tr = get_CV_target_encoding(tr_[cols_te], y[train_index], te, 5)

        te.fit(tr_[cols_te], y[train_index])

        data_te_te = te.transform(test[cols_te])

        data_te_val = te.transform(val_[cols_te])

        

        tr_ = pd.concat([

            data_train.iloc[train_index,:].reset_index(drop = True), 
        ...
        val_ = pd.concat([
            ...
            data_train.iloc[test_index,:].reset_index(drop = True), 
        ...
        test_ = pd.concat([data_test, data_te_te], axis = 1).values
        ...
        model.fit(
            ...
            eval_set = (val_, y[test_index]), 

            early_stopping_rounds = 25, eval_metric = 'auc', verbose = False

        )

        tst_preds.append(model.predict_proba(test_)[:,1].ravel())

    tst_preds = np.stack(tst_preds, -1).mean(-1)    

    

    predictions_2.append(tst_preds.ravel())
