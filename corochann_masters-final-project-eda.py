...
TEST_DIR = '../input/test'  # set for local environment!
# to reduce memory usage, dtype is specified.

train_df = pd.read_csv(os.path.join(DATA_DIR, 'train.csv'), dtype={
...
for i, f in enumerate(ld):
    ...
    df = pd.read_csv(os.path.join(TEST_DIR, f), dtype={'acoustic_data': np.int16})
