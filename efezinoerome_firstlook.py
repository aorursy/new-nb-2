...
print("Number of images and labels in the training data: {}  and {} respectively".\
      format(len(train_images), len(train_labels)))
train_images[200]
...
Image.open("../input/train_color/"+train_images[200])
im =Image.open("../input/train_color/"+train_images[200])
tlabel = np.asarray(Image.open("../input/train_label/"+train_labels[200])) // 1000
tlabel[tlabel!= 0] = 255
plt.imshow(Image.blend(im, Image.fromarray(tlabel).convert('RGB'), alpha=0.8))
display(plt.show())
np.unique(tlabel[tlabel>0])
tlabel.shape[1]/3
tlabel = np.asarray(Image.open("../input/train_label/"+train_labels[200])) // 1000
val = np.unique(tlabel)
#print(val)
