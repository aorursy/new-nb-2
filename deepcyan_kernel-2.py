...
#Kaggle has a different scikit, so this fails
...
#I kept the notebooks in one folder up
...
bclf = XGBRegressor(base_score=0.5, booster='gbtree', colsample_bylevel=1,
       ...
       random_state=276, reg_alpha=0, reg_lambda=1, scale_pos_weight=1,
...
bfit = bclf.fit(X.drop(['type'], axis = 1),Y)
...
pred = bclf.predict(test.drop(['type'], axis = 1)).reshape((-1,1))
