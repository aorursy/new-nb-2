...
#     add_lower(embed_fasttext, vocab)
    ...
    #Cleaning Contractions, Sepcial Characters and Misspellings
...
#     oov_fasttext = check_coverage(vocab, embed_fasttext)
    ...
    return df['treated_question'], data_y, embed_paragram

data_X, data_y, embedding = get_processed_data()
from tqdm import tqdm
def text_to_array(text):
    empyt_emb = np.zeros(300)
    text = text[:-1].split()[:30]
    embeds = [embedding.get(x, empyt_emb) for x in text]
