DATADIR = '../input/tensorflow-speech-recognition-challenge' # unzipped train data

TESTDIR = '../input/speech-recog-zip/test' # test data
...
def _create_my_experiment(run_config, hparams):
    ...
    exp = tf.contrib.learn.Experiment(
        ...
        train_steps=100, # just randomly selected params

        eval_steps=1,  # read source code for steps-epochs ariphmetics
...
paths = glob(os.path.join(TESTDIR, 'test/audio/*wav'))
...
with open(os.path.join('.', 'submission.csv'), 'w') as fout:
