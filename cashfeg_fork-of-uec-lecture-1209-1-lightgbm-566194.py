...
train.head()
train_data = lgb.Dataset(train.drop(["y", "index", "duration"], axis=1), label=train["y"])
...
bst = lgb.train(param, train_data, 25)
lgb.plot_importance(bst, figsize=(12, 6))
ypred = bst.predict(test.drop(["duration", "index"], axis=1))
sub = pd.read_csv("../input/ds2019uec-task1/sample_submission.csv")

sub["pred"] = ypred

sub.to_csv("sample_lgbm_drop2.csv", index=False)
train_y_pred = bst.predict(train.drop(["duration", "index", "y"], axis=1))
