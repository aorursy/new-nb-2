...
aisle_top_sort = aisle_top.sort_values(by='product_id', ascending=False)
...
### Before you move on to the final step, how can you ensure that the aisle_top has only 10 aisl
...
sns.barplot(aisle_top_sort.index, aisle_top_sort.product_id,order=aisle_top_sort.index)
plt.xlabel('aisle_id', size=15)
plt.ylabel('product_id', size=15)
# Modify the limits
plt.ylim(0,2000)
plt.show()
...
x=avg_pos_des.iloc[0:10,:]
sns.barplot(x.index,x.mean_add_to_cart_order,order=x.index)
