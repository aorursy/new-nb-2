...
def random_crop_img(img,crop_size):
    ...
    if size < crop_size:
        print('wrong crop size')    
        return 
    x=random.randint(0,img.shape[0]-crop_size-1)
    y=random.randint(0,img.shape[1]-crop_size-1)
...
# plt.imshow(img)
# plt.subplot(1,2,2)
# plt.imshow(random_crop_img(img,10))
# plt.show()
...
results = model.fit({'img': X_train, 'feat': X_feat_train}, y_train, batch_size=16, epochs=50, callbacks=callbacks,
                    validation_data=({'img': X_valid, 'feat': X_feat_valid}, y_valid),shuffle=True)
