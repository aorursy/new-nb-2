...
plt.xlabel('days_of_week', fontsize=10)
plt.title("Days' orders", fontsize=15)
plt.show()
orders_first = pd.DataFrame(orders[orders['order_number']==1])
orders_first.head()
orders_second = pd.DataFrame(orders[orders['order_number']==2])
orders_second.head()
