...
## 512x512 is quite slower. 

#And kaggle kernel only 3 hours. So i train on 9 epochs and save model to train on 9 epochs.
model.load_weights('../input/siim512x512/siim512B16-9epochs.h5')
# model.save('siim512B16-9epochs.h5')
CFG['batch_size'] = 16



cnt_test   = count_data_items(files_train)

steps      = cnt_test / (CFG['batch_size'] * REPLICAS) * CFG['tta_steps']

ds_testAug = get_dataset(files_train, CFG, augment=True, repeat=True, 
...
for i in range(5):
    ...
    submission.to_csv(f'train_model512B{i+2}.csv', index=False)
