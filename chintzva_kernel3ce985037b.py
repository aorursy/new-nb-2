...
X_val
model1 = GradientBoostingRegressor(loss='quantile',alpha=0.05)
...
((e1[e1>=0]*0.05).sum()+(e1[e1<0]*(0.05-1)).sum())/len(pred1)
model2 = GradientBoostingRegressor(loss='quantile',alpha=0.5)

model2.fit(X_train,y_train)
pred2=model2.predict(X_val)

pred2
pd.DataFrame(pred2).describe()
e2=y_val-pred2

((e2[e2>=0]*0.5).sum()+(e2[e2<0]*(0.5-1)).sum())/len(pred2)
model3 = GradientBoostingRegressor(loss='quantile',alpha=0.95)

model3.fit(X_train,y_train)
pred3=model3.predict(X_val)

pred3
pd.DataFrame(pred3).describe()
e3=y_val-pred3

((e3[e3>=0]*0.95).sum()+(e3[e3<0]*(0.95-1)).sum())/len(pred3)
...
test.head()
ans1=model1.predict(test)

ans2=model2.predict(test)

ans3=model3.predict(test)
sub=pd.read_csv('/kaggle/input/covid19-global-forecasting-week-5/submission.csv')
sub1=sub.copy()
for i in range(len(ans1)):

    sub1.iat[3*i,1]=ans1[i]

    sub1.iat[3*i+1,1]=ans2[i]

    sub1.iat[3*i+2,1]=ans3[i]
sub1.describe()
