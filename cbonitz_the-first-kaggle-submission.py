...
train_data.head(5)
train_data.hist(column='loss', bins=50)

fig = plt.figure()

log_loss = np.log(train_data['loss'])
...
for col in ["cont%d" % (x + 1) for x in range(14)]:

    print(col)

    plt.scatter(train_data[col],log_loss)

    plt.show()

    plt.scatter(train_data[col],train_data['loss'])
...
for col in train_data:
    ...
    if col.startswith('cont'):
        ...
        train_data.hist(column=col, bins=50)
...
#        train_data[col].astype('category').value_counts().plot(kind='bar')
